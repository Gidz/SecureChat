        ...
        public void run() {
            ...
            while(true) {
                ...
                try {
                    ...
                    {
                        AES aes = new AES(User.sharedSecretKey);
                        System.out.println("> "+aes.decrypt(message.getEncryptedMessage()));
                    }
                    else if(messageType.equals("UPDATE_NODE_NUMBER"))
                    {
                        User.NODE_NUMBER = Integer.parseInt(message.getMessage());
                        System.out.println("The securechat.node number is "+ User.NODE_NUMBER);
                    }
...
//                System.out.println("> "+toHexString(message.getEncryptedMessage()));
                        ...
                        User.myDHKeyPair = node2KpairGen.generateKeyPair();

                        // Node2 creates and initializes DH KeyAgreement object
                        User.myKeyAgreement = KeyAgreement.getInstance("DH");
                        User.myKeyAgreement.init(User.myDHKeyPair.getPrivate());

                        // Node2 encodes its public key, and sends it over to Node1.
                        byte[] publicKey = User.myDHKeyPair.getPublic().getEncoded();
//                System.out.println("Sending this over the network "+Node.myDHKeyPair.getPublic());

                        //Meanwhile calulate the shared secret key
                        User.myKeyAgreement.doPhase(node1PubKey, true);
                        User.sharedSecretKey = User.myKeyAgreement.generateSecret();
//                System.out.println("The shared secret key is "+toHexString(Node.sharedSecretKey));


                        User.sendMessage(new Message("DH2",publicKey, User.NODE_NUMBER), User.TTP_PORT);
                    }
...
//                System.out.println("> "+toHexString(message.getEncryptedMessage()));
                        ...
                        User.myKeyAgreement.doPhase(node2PublicKey, true);
                        User.sharedSecretKey = User.myKeyAgreement.generateSecret();
//                System.out.println("The shared secret key is "+toHexString(Node.sharedSecretKey));
                        User.sendMessage(new Message("START_CHAT",""), User.TTP_PORT);
                    }
